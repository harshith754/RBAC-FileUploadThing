  ...
  return (
    <div className="space-y-2">
      {uploadQueue.map((item) => {
        const isCompletedWithUrl = item.status === "completed" && item.url;
        const handleClick = () => {
          if (isCompletedWithUrl) {
        ...
        return (
          <Card
            key={item.id}
            className={`pt-0 pb-0 ${
              isCompletedWithUrl ? "cursor-pointer" : ""
            }`}
            onClick={handleClick}
            tabIndex={isCompletedWithUrl ? 0 : -1}
            role={isCompletedWithUrl ? "button" : undefined}
            aria-label={isCompletedWithUrl ? `Open ${item.name}` : undefined}
          >
            <CardContent
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-2 min-w-0">
                      <div className="flex items-center gap-1 font-medium text-foreground truncate text-xs sm:text-base max-w-[120px] sm:max-w-xs">
                        ...
                        {isCompletedWithUrl ? (
                          <Button
                            variant="link"
                            className="p-0 text-blue-600 hover:underline truncate text-left h-auto cursor-pointer"
                            onClick={(e) => {
                              e.stopPropagation();
                              window.open(item.url!, "_blank");
                            }}
